...
int GetInputActive(int control) {
    if (control == CONTROL_MOVE && 
        (GetInputActive(CONTROL_LEFT) || GetInputActive(CONTROL_RIGHT)))
        return 1;
    if (control == CONTROL_PAUSE && 
        (IsKeyDown(KEYBIND_PAUSE) || 
        IsGamepadButtonDown(0, GAMEPAD_PAUSE)))
        return 1;
    if (control == CONTROL_JUMP && 
        (IsKeyDown(KEYBIND_JUMP) || 
        IsGamepadButtonDown(0, GAMEPAD_JUMP)))
        return 1;
    ...
    if (control == CONTROL_LEFT && 
        ...
        IsGamepadButtonDown(0, GAMEPAD_LEFT)))
        return 1;
    if (control == CONTROL_RIGHT && 
        (IsKeyDown(KEYBIND_RIGHT) || 
        IsGamepadButtonDown(0, GAMEPAD_RIGHT)))
        return 1;
    return 0;
}
